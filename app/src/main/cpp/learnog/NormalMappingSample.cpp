...
NormalMappingSample::NormalMappingSample() {
    VBO = GL_NONE;
...
void NormalMappingSample::loadImage(NativeImage *pImage) {
...
void NormalMappingSample::loadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGE("NormalMappingSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
        if (0 == index) {
 ...
 * @param ratio 宽高比
 * */
void NormalMappingSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("NormalMappingSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    // Projection matrix
...
void NormalMappingSample::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
