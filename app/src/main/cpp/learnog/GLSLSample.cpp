...
void GLSLSample::loadImage(NativeImage *pImage) {
    LOGE("GLSLSample::LoadImage pImage = %p", pImage->ppPlane[0]);
}

void GLSLSample::draw(int screenW, int screenH) {
    if(shader == nullptr) return;
    LOGE("GLSLSample::Draw()");
...
void GLSLSample::destroy() {
    LOGE("GLSLSample::Destroy");
...
void GLSLSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("GLSLSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
...
void GLSLSample::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
    GLSampleBase::updateTransformMatrix(rotateX, rotateY, scaleX, scaleY);
    LOGE("GLSLSample::updateTransformMatrix, rotateX = %f", rotateX);
