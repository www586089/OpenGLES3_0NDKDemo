...
ShadowMappingSample::ShadowMappingSample() {
    cubeVAO = GL_NONE;
    cubeVBO = GL_NONE;
}
...
//            LOGE("ShadowMappingSample::init glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE");
//        }
//        glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);
        ...
        LOGE("ShadowMappingSample::Init create program fail");
        return;
    }
}


void ShadowMappingSample::initShadowMap() {
...
void ShadowMappingSample::draw(int screenW, int screenH) {
    LOGE("ShadowMappingSample::Draw()");

    if (!simpleDepthShader.isAvailable()) {
        LOGE("ShadowMappingSample::Draw() return");
        return;
    }
    if (firstFrame) {
        LOGE("ShadowMappingSample::Draw() config fbo, depthMapFBO = %d.", depthMapFBO);
        firstFrame = false;
...
//        glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);
    ...
    LOGE("ShadowMappingSample::Do Draw()");
...
// -------------------------------------------------
void ShadowMappingSample::renderCube() {
...
// -----------------------------------------
void ShadowMappingSample::renderQuad() {
    if (quadVAO == 0) {
        float quadVertices[] = {
...
void ShadowMappingSample::loadImage(NativeImage *pImage) {
    LOGE("ShadowMappingSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void ShadowMappingSample::loadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGE("ShadowMappingSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
        if (0 == index) {
...
void ShadowMappingSample::destroy() {
    if (simpleDepthShader.isAvailable()) {
 ...
 * @param ratio 宽高比
 * */
void ShadowMappingSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("ShadowMappingSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    // Projection matrix
...
void ShadowMappingSample::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
