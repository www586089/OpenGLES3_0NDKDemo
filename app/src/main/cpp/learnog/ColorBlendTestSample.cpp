...
void ColorBlendTestSample::init() {
    ...
    if (shader.isAvailable()) {
        ...
        LOGE("ColorBlendTestSample::Init create program fail");
        return;
    }
}
...
void ColorBlendTestSample::draw(int screenW, int screenH) {
    LOGE("ColorBlendTestSample::Draw()");

    if (!shader.isAvailable()) {
        LOGE("ColorBlendTestSample::Draw() return");
        return;
    }
    LOGE("ColorBlendTestSample::Do Draw()");
    // render
    // ------
...
void ColorBlendTestSample::loadImage(NativeImage *pImage) {
    LOGE("ColorBlendTestSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void ColorBlendTestSample::loadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGE("ColorBlendTestSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
        if (0 == index) {
...
void ColorBlendTestSample::destroy() {
    if (shader.isAvailable()) {
 ...
 * @param ratio 宽高比
 * */
void ColorBlendTestSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("ColorBlendTestSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    // Projection matrix
...
void ColorBlendTestSample::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
