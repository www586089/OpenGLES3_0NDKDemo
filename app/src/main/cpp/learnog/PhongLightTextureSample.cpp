...
#include "PhongLightTextureSample.h"
#include <gtc/matrix_transform.hpp>
#include "../utils/GLUtils.h"
#include "Camera.h"
#include "Shader.h"

PhongLightTextureSample::PhongLightTextureSample() {
    VBO = GL_NONE;
...
void PhongLightTextureSample::init() {
    ...
    if (lightingShader.isAvailable() && lightCubeShader.isAvailable()) {
        ...
        LOGE("PhongLightTextureSample::Init create program fail");
        return;
    }
}
...
void PhongLightTextureSample::draw(int screenW, int screenH) {
    ...
    if (!lightingShader.isAvailable() || !lightCubeShader.isAvailable()) {
        LOGE("PhongLightTextureSample::Draw() return");
        return;
    }
    LOGE("PhongLightTextureSample::Do Draw()");
    ...
    for(int i= 0; i < 36; i++) {
        LOGE("PhongLightTextureSample::Draw() read feedback buffer Normal[%d] = [%f, %f, %f]",
...
void PhongLightTextureSample::destroy() {
    if (lightingShader.isAvailable()) {
 ...
 * @param ratio 宽高比
 * */
void PhongLightTextureSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("PhongLightTextureSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    // Projection matrix
...
void PhongLightTextureSample::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY) {
