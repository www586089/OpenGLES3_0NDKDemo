...
void FBOSample::loadImage(NativeImage *pImage) {
    LOGE("FBOSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void FBOSample::init() {
    ...
    if (!CreateFrameBufferObj()) {
        LOGE("FBOSample::Init CreateFrameBufferObj fail");
        return;
    }
...
void FBOSample::draw(int screenW, int screenH) {
    ...
    // Do FBO off screen rendering Begin
    // 1. 绑定 FBO
    glBindFramebuffer(GL_FRAMEBUFFER, m_FboId);
    // 2. 选定离屏渲染的 Program，绑定 VAO 和图像纹理，进行绘制（离屏渲染）。m_ImageTextureId 为另外一个用于纹理映射的图片纹理
    ...
    // dump begin
//	uint8_t *pBuffer = new uint8_t[mRenderImage.width * mRenderImage.height * 4];
//
//	NativeImage nativeImage = mRenderImage;
