...
{
    LOGE("TransformFeedbackSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage)
    {
...
void TransformFeedbackSample::init()
{
    ...
    m_ProgramObj = GLUtils::CreateProgramWithFeedback(vShaderStr, fShaderStr, m_VertexShader, m_FragmentShader, varyings,sizeof(varyings) / sizeof(varyings[0]));
...
void TransformFeedbackSample::draw(int screenW, int screenH) {
    ...
    for(int i= 0; i< 6; i++) {
        LOGE("TransformFeedbackSample::Draw() read feedback buffer outPos[%d] = [%f, %f, %f], outTex[%d] = [%f, %f]",
