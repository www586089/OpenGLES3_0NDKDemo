...
void FBOLegLengthenSample::loadImage(NativeImage *pImage) {
    LOGE("FBOLegLengthenSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
//	};
    ...
    if (m_FboProgramObj) {
        ...
        switch (m_StretchMode) {
            ...
            case HORIZONTAL_STRETCH_RIGHT_6_POINTS:
                ...
                break;
            default:
                LOGE("FBOLegLengthenSample::Init don't support this mode");
                return;
        }
        ...
        if (m_bIsVerticalMode) {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA,
                         static_cast<GLsizei>(m_RenderImage.width * (1 + 2 * m_dt)), m_RenderImage.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        }

        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            LOGE("FBOLegLengthenSample::Init glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE");
        }
        glBindTexture(GL_TEXTURE_2D, GL_NONE);
    ...
    m_FboProgramObj = CreateProgram(vFboShaderStr, fFboShaderStr, m_FboVertexShader,
                                    m_FboFragmentShader);

    if (m_ProgramObj == GL_NONE || m_FboProgramObj == GL_NONE) {
        LOGE("FBOLegLengthenSample::Init m_ProgramObj == GL_NONE");
        return;
    }
    ...
    if (!CreateFrameBufferObj()) {
        LOGE("FBOLegLengthenSample::Init CreateFrameBufferObj fail");
        return;
    }
}

void FBOLegLengthenSample::draw(int screenW, int screenH) {
    LOGE("FBOLegLengthenSample::Draw [screenW, screenH] = [%d, %d]", screenW, screenH);
...
bool FBOLegLengthenSample::CreateFrameBufferObj() {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        LOGE("FBOLegLengthenSample::CreateFrameBufferObj glCheckFramebufferStatus status != GL_FRAMEBUFFER_COMPLETE");
        return false;
    }
    glBindTexture(GL_TEXTURE_2D, GL_NONE);
    glBindFramebuffer(GL_FRAMEBUFFER, GL_NONE);
