...
void BlendingSample::init()
{
    ...
    if (m_ProgramObj) {
        ...
        LOGE("BlendingSample::Init create program fail");
        return;
    }
...
void BlendingSample::loadMultiImageWithIndex(int index, NativeImage *pImage) {
    ...
    LOGE("BlendingSample::LoadImage index=%d, pImage = %p", index, pImage->ppPlane[0]);
    if (pImage) {
...
{
    LOGE("BlendingSample::UpdateMatrix angleX = %d, angleY = %d, ratio = %f", angleXRotate,
            angleYRotate, ratio);
