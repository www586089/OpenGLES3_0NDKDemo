...
void SkyBoxSample::init()
{
    ...
    if (m_ProgramObj) {
        ...
        LOGE("SkyBoxSample::Init create program fail");
        return;
    }
...
void SkyBoxSample::loadImage(NativeImage *pImage) {
    LOGE("SkyBoxSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void SkyBoxSample::loadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGE("SkyBoxSample::LoadMultiImageWithIndex pImage = %p, index=%d", pImage->ppPlane[0], index);
    if (pImage) {
...
void SkyBoxSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float scale, float ratio){
    LOGE("SkyBoxSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
