...
void BasicLightingSample::init() {
    ...
    if (m_ProgramObj) {
        ...
        LOGE("BasicLightingSample::Init create program fail");
        return;
    }
...
void BasicLightingSample::loadImage(NativeImage *pImage) {
    LOGE("BasicLightingSample::LoadImage pImage = %p", pImage->ppPlane[0]);
    if (pImage) {
...
void BasicLightingSample::draw(int screenW, int screenH) {
    ...
    glUniform3f(m_LightPosLoc,    0.0f, 3.0f, 0.0f);
    glUniform3f(m_ViewPosLoc,     2.0f, 2.0f, 2.0f);
...
void BasicLightingSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGE("BasicLightingSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX, angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
